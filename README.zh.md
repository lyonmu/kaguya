...
| 对话管理 | 继续已保存的对话，按标题前缀搜索，收藏、重命名和删除对话，分页浏览历史并查看轮次摘要。 |
...
通过左侧列表新建或打开历史对话，按标题前缀搜索，或只查看收藏。对话顶部提供收藏、重命名和删除操作。输入框可选择提供商及模型，也可使用默认模型：**Enter** 发送，**Shift + Enter** 换行，生成中点击 **停止** 可取消回答。
