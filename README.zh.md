...
只有成功完成并保存的轮次会成为可继续使用的对话历史。失败或取消时的部分回答不会保存为完整轮次。配置后台任务模型后，新会话在首轮正文开始生成前即创建并出现在列表中，同时只用用户提问并行请求简短中文标题，模型返回后异步更新；失败或取消的轮次保留空会话，也不会覆盖手动设置的标题。
...
两者的防护范围不同：**外部密钥保存在数据库之外**，数据库文件、备份或导出泄露时密文仍不可读；**证书派生密钥与密文同在数据库中**，只能防止明文直接出现在库文件和导出结果里，不能抵御整库窃取。轮换 TLS 证书会改变派生密钥，此时 `TLSUpdate` 会先用旧密钥解密、再用新密钥重新加密所有已存储的 API Key；若证书由外部工具替换而数据库未同步，需重新填写 API Key：提供商管理接口返回 `106006`，对话与标题接口返回 `102010`。升级既有安装时，首次启动会自动把历史明文记录就地转为密文，该操作可重复执行。
