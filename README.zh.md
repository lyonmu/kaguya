...
2. 点击左下角 **系统管理**，进入 **AI 配置 → 提供商与模型**：可以直接自定义提供商的名称、API 根地址和 API Key，也可以先在 **提供商目录** 中选择 models.dev 收录的提供商并预填表单。
3. 在 **系统配置** 中手动或定时同步 [models.dev](https://models.dev/api.json) 目录，或直接在 **AI 配置 → 提供商目录 / 模型目录** 点击同步；两份目录来自同一份 api.json。**提供商目录** 只收录带 API 根地址的提供商并按名称 A→Z 排列，**模型目录** 是全量模型并按发布日期从新到旧排列。进入提供商的 **模型管理** 选择目录模型，会自动填充标识、请求协议、Token 上限与能力；提供商有差异时仍可编辑。
...
- **提供商与模型**：自定义或编辑提供商与模型；提供商只保存名称、API 根地址与 API Key。
- **提供商目录**：来自 models.dev 的提供商根地址，按名称 A→Z 排列，支持按名称、标识、包名或 API 地址检索；选择后预填新建表单，也可以改成自定义名称。
- **模型目录**：全量模型目录（包含没有 API 根地址的提供商），按发布日期从新到旧排列，支持按名称、标识、提供商、系列或描述检索。
...
模型协议配置在**模型**上，同一个提供商可以同时提供多种协议：

| 模型协议 | 配置值 | 追加路径 | 最终请求地址示例 |
| --- | --- | --- | --- |
...
提供商只填写到 API 版本段的根地址（如 `https://api.example.com/v1`），应用按模型的协议追加端点路径，不重复或缺少版本段；根地址不能带查询参数或片段。提供商类型支持普通类型 `normal` 和带 OpenCode 会话头的 `opencode-go`。

模型配置包含显示名称、上游模型标识、请求协议、思考等级、上下文窗口、最大输出 Token，以及 Tool/Vision/JSON 能力元数据。从目录选择模型时会按 models.dev 提供商的 npm 包预选协议（Anthropic 包 → Message，OpenAI 兼容包 → Chat），但不保证当前提供商一定开放该模型；仍需核对提供商可用性并按需调整。项目与 MCP 工具调用依赖模型支持工具，图片读取还依赖视觉能力。
...
| 目录同步 | 可配置完整 HTTP(S) 目录地址（默认 `https://models.dev/api.json`），手动同步或启用 `1–720` 小时的周期同步，一次同时更新提供商目录与全量模型目录；“AI 配置 → 提供商目录”按名称 A→Z 显示，“模型目录”支持按名称、标识、提供商、系列或描述检索并按发布日期从新到旧显示 |
