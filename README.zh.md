...
2. 点击左下角 **系统管理**，进入 **AI 配置 → 提供商与模型**：新增提供商时可以直接搜索 **提供商目录**（models.dev api.json）预填名称与 Base URL，再填写 API Key；也可以手动输入自定义名称、Base URL 和 API Key。
3. 在 **系统配置** 中手动或定时同步 [models.dev](https://models.dev) 目录，或直接在 **AI 配置 → 提供商目录 / 模型目录** 点击同步。提供商目录来自 `api.json`，只收录带 API 地址的提供商并按名称 A→Z 排列；模型目录来自 `models.json`，已按模型去重并按发布日期从新到旧排列。进入提供商的 **模型管理** 选择目录模型，会自动填充标识、Token 上限与能力；请求协议与路径按默认值带出，提供商有差异时仍可编辑。
...
| `grep` | 用主机 ripgrep 搜索文件内容，可按文件名过滤，并支持大小写、纯文本与上下文行 |
| `find` | 用主机 fd 按 glob 递归查找路径 |
| `ls` | 列出单个目录的直接子项 |

`grep`、`find`、`ls` 为只读工具，依赖主机上的 `rg` 与 `fd`，遵循 `.gitignore` 并包含隐藏文件。
...
入口：**系统管理 → AI 配置**，包含 **提供商与模型**、**提供商目录** 与 **模型目录** 三个标签。

- **提供商与模型**：自定义或编辑提供商与模型；提供商只保存名称、Base URL 与 API Key，新增时可从提供商目录搜索预填。
- **提供商目录**：来自 models.dev 的提供商目录（api.json），按名称 A→Z 排列，支持按名称、标识、包名或 API 地址检索；新增提供商在「提供商与模型」中搜索完成。
- **模型目录**：来自 models.dev 的模型目录（models.json），已按模型去重，按发布日期从新到旧排列，支持按名称、标识、提供商、系列或描述检索。
...
模型协议配置在**模型**上，同一个提供商可以同时提供多种协议；请求协议决定运行时使用哪套请求实现：

| 模型协议 | 配置值 | 默认请求路径 | 最终请求地址示例 |
| --- | --- | --- | --- |
...
提供商 Base URL 按字符串原样保存（通常直接取提供商目录中的 API 地址），请求地址 = Base URL + 模型请求路径；请求路径默认是协议后缀常量（`/chat/completions`、`/responses`、`/messages`），可改成任意完整路径，例如 DeepSeek 的 Anthropic 兼容端点写 `/anthropic/v1/messages`。切换协议会自动填入默认路径。提供商类型支持普通类型 `normal` 和带 OpenCode 会话头的 `opencode-go`。

模型配置包含显示名称、上游模型标识、请求协议、请求路径、思考等级、上下文窗口、最大输出 Token，以及 Tool/Vision/JSON 能力元数据。从目录选择模型会填充名称、标识与能力元数据，请求路径由协议默认值带出；目录不保证当前提供商一定开放该模型，仍需核对可用性并按需调整。项目与 MCP 工具调用依赖模型支持工具，图片读取还依赖视觉能力。

新增或编辑模型时先用弹窗中的 **测试** 按钮验证可用性：后端用当前表单配置向该模型发送一条 `Hi!` 请求，只有测试通过后才可保存；改动提供商、模型标识、请求协议或请求路径后需要重新测试。
...
| 目录同步 | 提供商目录默认 `https://models.dev/api.json`，模型目录默认 `https://models.dev/models.json`，均为完整 HTTP(S) 地址；手动同步或启用 `1–720` 小时的周期同步，一次同时更新两份目录。“AI 配置 → 提供商目录”按名称 A→Z 显示，“模型目录”支持按名称、标识、提供商、系列或描述检索并按发布日期从新到旧显示 |
