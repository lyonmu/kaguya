...
- 当前应用按 **macOS Desktop 默认 + 可选 HTTP 服务 + SQLCipher 单后端** 使用：macOS 无参数启动为原生窗口，`--web` 启动原 HTTP 服务（`--host`／`--port`／`--trusted-host` 仅对 `--web` 生效）。`make install` 完整构建并安装二进制到 `~/.local/bin/<仓库目录名>`，`make package-macos` 组装 `target/Kaguya.app`，`make dmg-macos` 打包可拖拽安装的 `target/Kaguya-<版本>.dmg`。数据库是唯一运行时存储，网络数据库（MySQL/PostgreSQL）与 Redis 代码已移除；主机须提供 Bash 和项目工具链，默认不构建或测试 Docker。
