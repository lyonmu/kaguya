...
func (KaguyaModelsInfo) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("api_protocol").Optional().GoType(consts.ProviderProtocol("")).Default(string(consts.ProtocolOpenAIChat)).Comment("请求协议，决定运行时使用哪套请求实现"),
		field.String("request_path").Default("").Comment("模型请求路径，与提供商 BaseURL 拼接成最终请求地址"),
