...
func (KaguyaSystemInfo) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Bool("model_sync_enabled").Default(false).Comment("是否定时同步 models.dev 模型目录"),
		field.String("model_sync_url").Default(consts.DefaultModelCatalogURL).MaxLen(2048).Comment("模型目录同步地址"),
