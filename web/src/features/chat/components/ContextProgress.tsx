...
export function ContextProgress({ conversationId, turnCount }: { conversationId?: string; turnCount?: number }) {
  ...
  useEffect(() => {
    ...
    if (conversationId) {
      fetchConversationContext(conversationId, controller.signal).then(result => {
        ...
        if (!controller.signal.aborted) setError(error instanceof Error ? error.message : '累计用量加载失败')
  ...
  const title = loading ? '正在加载累计用量…' : error ? <>{error} <Button size="small" onClick={() => setRevision(value => value + 1)}>重试</Button></> : !conversationId ? '首轮对话结束后显示累计 token 占比' : <>
    <div>{data?.model_name || '最后一轮模型'} · 会话累计 token 占比</div>
    ...
    <div>累计所有已完成轮次（含跨模型）的 total_tokens，按最近完成轮次模型的窗口计算；不代表实际上下文占用，不会自动裁剪消息。</div>
  </>
  return <Tooltip title={title}>
    <span className="chat-context-progress" tabIndex={0} aria-label={known ? `累计 token 占比 ${percent.toFixed(1)}%` : '累计 token 占比未知'}>
