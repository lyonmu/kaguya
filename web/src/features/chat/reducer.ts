...
export function applyFrame(turn: Turn, frame: ChatFrame): Turn {
  ...
  if (frame.chat.flag === 'done') {
    return { ...next, usage: frame.usage, status: 'done', finished_at: new Date().toISOString(), duration_ms: Date.now() - Date.parse(turn.started_at) }
  }
  ...
  if (event.type === 'tool_call' || event.type === 'tool_result') {
    ...
    const block: Block = index < 0 ? { ...event, type: 'tool_call', input: '' } : blocks[index]
    if (event.type === 'tool_result') {
      Object.assign(block, { output: event.output, is_error: event.is_error, error_message: event.error_message, phase: 'block_end' })
    } else {
