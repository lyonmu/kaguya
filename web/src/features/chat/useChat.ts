...
import { fetchConversation, fetchTurns, streamChat } from './api'
...
export function useChat(onCompleted: () => Promise<void>, onTitleUpdated: (title: ConversationTitle) => void) {
  ...
  const select = useCallback(async (nextId: string) => {
    ...
    if (!nextId) return
    try {
      const [detail, history] = await Promise.all([fetchConversation(nextId, controller.signal), fetchTurns(nextId, 0, controller.signal)])
      if (token !== generation.current) return
      setConversation(detail)
      setTurns(history.items ?? [])
      setHasMore(history.has_more)
      before.current = history.next_before
    } catch (error) {
  ...
  const loadMore = async () => {
    if (loading || streaming || !hasMore) return
    const token = generation.current
    const controller = new AbortController()
    request.current = controller
    setLoading(true)
    try {
      const history = await fetchTurns(id, before.current, controller.signal)
      if (token !== generation.current) return
      setTurns(current => [...(history.items ?? []), ...current])
      setHasMore(history.has_more)
      before.current = history.next_before
    } catch (error) {
      if (!controller.signal.aborted) setError(errorText(error))
  ...
  const send = async (text: string, modelId?: string) => {
    if (!text.trim() || stream.current || loading) return
    const controller = new AbortController()
    ...
    let completed = false
    const turn: Turn = {
      turn_index: (turns.at(-1)?.turn_index ?? 0) + 1,
  ...
  return { id, viewKey, conversation, setConversation, turns, loading, streaming, error, hasMore, select, loadMore, send, cancelTitleWait, stop: () => stream.current?.abort() }
}
