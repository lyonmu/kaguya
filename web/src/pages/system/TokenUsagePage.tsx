...
export function TokenUsagePage() {
  ...
  const [compositionWidth, setCompositionWidth] = useState(0)
  ...
  const measureComposition = useCallback((element: HTMLDivElement | null) => {
    if (!element) return
    const observer = new ResizeObserver(entries => setCompositionWidth(entries[0].contentRect.width))
    observer.observe(element)
    return () => observer.disconnect()
  }, [])

  const compositionRows = useMemo(() => data ? (dimension === 'model' ? data.models : data.providers) : [], [data, dimension])
  const compositionLabel = compositionLabelWidth(compositionWidth, compositionRows.length)
