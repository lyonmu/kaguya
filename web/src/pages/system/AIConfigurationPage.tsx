...
const ProviderCatalogPage = lazy(() => import('./ProviderCatalogPage').then(module => ({ default: module.ProviderCatalogPage })))
const ModelCatalogPage = lazy(() => import('./ModelCatalogPage').then(module => ({ default: module.ModelCatalogPage })))
...
export function AIConfigurationPage() {
  return <Suspense fallback={<div className="p-8 text-center"><Spin /></div>}>
    <Tabs className="w-full" tabBarStyle={{ padding: '8px 24px 0', marginBottom: 0 }} destroyOnHidden items={[
      ...
      { key: 'provider-catalog', label: '提供商目录', children: <ProviderCatalogPage /> },
      { key: 'model-catalog', label: '模型目录', children: <ModelCatalogPage /> },
