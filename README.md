...
Only successfully completed and saved turns become reusable conversation history. Failed or canceled partial replies are not saved as completed turns. With a background-task model configured, a new conversation is created and listed as soon as its first turn starts generating, and a short Chinese title is requested in parallel using the user question alone (without waiting for the answer) and written asynchronously once the model responds; failed or canceled turns keep an empty conversation and never overwrite manual titles.
